...
const initializeFlatContainer = (
  bruteState: BruteState,
  baseContainer: PartContainer,
  currentContainer: PartContainer,
  symbol: Symbol,
  colorIdx?: string,
) => {
  console.log('initializeFlatContainer', currentContainer.name);
  // Only initialize max number of SVGs needed, not sum of all SVGs
  const neededSvgs: Record<string, number> = {};

  // Only one needed frame except for weapons and base
  const neededFrames = (symbol.name === WEAPON_SYMBOL || symbol.name === baseContainer.name)
    ? Array(weaponFrames.length).fill(0).map((_, i) => i)
    : [0];

  neededFrames.forEach((frameIndex) => {
    const frame = symbol.frames?.[frameIndex];

    if (!frame) {
      throw new Error(`Frame ${frameIndex} not found`);
    }

    // TODO: This whole svg memo doesn't work, fix it
    const usedSvgs: Record<string, number> = {};

    frame.forEach((framePart) => {
      if (framePart.type === 'symbol') {
        const container: PartContainer = new PIXI.Container();
        container.name = framePart.name;
        container.visible = false;

        // Get part corresponding to the frame part
        const part = symbol.parts?.find(part => part.name === framePart.name && part.type === framePart.type);

        if (!part) {
          throw new Error(`Part ${framePart.type}:${framePart.name} not found`);
        }

        if (part.type !== 'symbol') {
          throw new Error(`Part ${framePart.type}:${framePart.name} is not a symbol`);
        }

        currentContainer.addChild(container);

        // Recursively initialize parts
        const innerNeededSvgs = initializeFlatContainer(
          bruteState,
          baseContainer,
          container,
          part,
          part.colorIdx ?? colorIdx,
        );

        // Update used parts
        Object.keys(innerNeededSvgs).forEach(part => {
          usedSvgs[part] = Math.max(usedSvgs[part] || 0, innerNeededSvgs[part]);
        });
      } else {
        usedSvgs[framePart.name] = (usedSvgs[framePart.name] || 0) + 1;
      }
    });

    // Update needed parts
    Object.keys(usedSvgs).forEach(part => {
      neededSvgs[part] = Math.max(neededSvgs[part] || 0, usedSvgs[part]);
    });
  });

  // Initialize all parts
  Object.keys(neededSvgs).forEach(neededSvg => {
    // Get part corresponding to the needed SVG
    const part = symbol.parts?.find(part => part.name === neededSvg && part.type === 'svg');

    if (!part) {
      throw new Error(`Part svg:${neededSvg} not found`);
    }

    if (part.type !== 'svg') {
      throw new Error(`Part svg:${neededSvg} is not a svg`);
    }
    ...
    baseContainer.addChild(svg);
  });

  console.log(neededSvgs);

  return neededSvgs;
};

const organizeSVGsForFrame = (
  bruteState: BruteState,
  baseContainer: PartContainer,
  ...
  console.log(`Organizing ${symbol.name} for frame ${frame}`)
  // Hide all parts
  ...
  // Get frame
  const frameParts = symbol.frames?.[frame];

  const usedSymbols: Record<string, number> = {};

  frameParts?.forEach((framePart) => {
    console.log('framePart', framePart);
    console.log('container children', container.children);
    const partSymbol = symbol.parts?.filter(part => part.name === framePart.name && part.type === framePart.type)[usedSymbols[framePart.name] || 0];
    const framePartContainer = container.children.filter((child) => child.name === framePart.name)[usedSymbols[framePart.name] || 0] as PartContainer | undefined;

    if (!partSymbol) {
      throw new Error(`Part symbol ${framePart.type}:${framePart.name} not found`);
    }
    if (!framePartContainer) {
      throw new Error(`Part ${framePart.type}:${framePart.name} not found`);
    }

    usedSymbols[framePart.name] = (usedSymbols[framePart.name] || 0) + 1;
    ...
    if (framePart.maskedBy) {
      const mask = baseContainer.children.find(child => child.name === `Symbol${framePart.maskedBy}`) as PIXI.Sprite | undefined;

      if (!mask) {
        throw new Error(`Mask ${framePart.maskedBy} not found`);
      }
  
      framePartContainer.addChild(mask);
      framePartContainer.mask = mask;
    }
    ...
    // Select correct weapon frame
    if (framePart.type === 'symbol' && framePart.name === WEAPON_SYMBOL) {
      const weaponFrame = weaponFrames.findIndex(weapon => weapon === bruteState.weapon);
      if (weaponFrame >= 0) {
        frameToDisplay = weaponFrame;
      }
    ...
    // Hide shield if needed
    if (framePart.type === 'symbol' && framePart.name === SHIELD_SYMBOL) {
      framePartContainer.visible = bruteState.shield;
    }
  
    // Display correct part
    if (partSymbol.type === 'symbol' && partSymbol.partIdx) {
      const partIdx = bruteState.parts[partSymbol.partIdx];
  
      if (partIdx === undefined) {
        throw new Error(`Part ${partSymbol.partIdx} not found`);
      }
  
      frameToDisplay = partIdx;
    }
  
    // Recursive on symbols
    if (partSymbol.type === 'symbol' && partSymbol.frames?.[frameToDisplay]) {
      organizeSVGsForFrame(bruteState, baseContainer, framePartContainer, partSymbol, frameToDisplay);
    }

    if (partSymbol.type === 'svg') {
      // Move SVG to the correct container
      container.addChild(framePartContainer);
    }
  });
};
...
const displaySymbol = (bruteState: BruteState, x?: number, y?: number) => {
  ...
  // symbol.frames?.[bruteState.frame]?.forEach((part, i) => {
  //   // Get amount of same symbols
  //   const twinParts = symbol.frames?.[bruteState.frame].filter((p, j) => p.name === part.name && p.type === part.type && j < i);

  //   displayPart(bruteState, symbolContainer.children, part, twinParts?.length || 0);
  // });

  // Initialize parts and containers
  initializeFlatContainer(bruteState, symbolContainer, symbolContainer, symbol);

  console.log('symbolContainer', symbolContainer);
  // Move SVGs to their correct container for the current frame
  organizeSVGsForFrame(bruteState, symbolContainer, symbolContainer, symbol, bruteState.frame);
